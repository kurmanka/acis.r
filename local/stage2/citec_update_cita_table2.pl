...
sub process_a_document {
  ...
  my $refcount = $cite_h->execute($srcdocid,$doctitle) or die "can't execute the query: $cite_h->errstr";
  if ($refcount and +$refcount==0) {
    #logger(1, "by title: no reference found (title:$doctitle,like $sDocKeyTitle)", $log_file);
  }

  if ($refcount and +$refcount==0 and length($doctitle) > 20 ) {
    my $title = $doctitle;
    $title =~ s/\s+/ /g;
    my $titlestart = substr( $title, 0, 15 );
    $cite_h = $dbh->prepare("select titulo,docid,year,id,autor from REFERENCIA where docid=? and titulo like ?");
    $refcount = $cite_h->execute($srcdocid,"$titlestart\%") or die "can't execute the query: $cite_h->errstr";
    if ($refcount and +$refcount==0) {
      logger(1, "by titlestart: no reference found (titlestart:$titlestart\%)", $log_file);
    }
  }
  ...
  while ( @cite = $cite_h->fetchrow_array ) {
    ...
    foreach my $citehandle (@aCitesFound) { 
      logger(1, "This doc has been cited by $refid, it seems", $log_file);
      next; # XXXXXXX


      if(careful) { # this is really optional:
