 <xsl:stylesheet 
  ...
  <xsl:template name='name-variations-display'>
    <p>
      <xsl:text>List of names we searched (based on your full name and </xsl:text>
      <a ref='@name?back=research#variations'>
      <xsl:text>variations</xsl:text></a>
      <xsl:text>):</xsl:text>
    </p>
    <p class='pad'>
      <a ref='@name?back=research#variations'
  ...
  <xsl:template name='search-form'>
    ...
    <acis:form screen='@research/search' id='searchform'>
  ...
  <xsl:variable name='name-variations-linked' xml:space='preserve'>
    <ul style='margin-bottom: 0;' class='names nameVariations' id='nameVariationsList'>
      <xsl:for-each select='//autosearch/names-list-nice/list-item'>
        <li class='name'><a ref='@name?back={$request-screen}#variations'  class='hidden'>
  ...
  <xsl:variable name='search-status'>
    <xsl:choose>
      <xsl:when test='$back-search-started'>
        ...
        While the system searches, you shall be able to see what's found and
  ...
  <xsl:template name='run-automatic-search-form'>
      <acis:form screen='@research/autosuggest' xsl:use-attribute-sets='form' class='narrow'>
